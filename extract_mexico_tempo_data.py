#!/usr/bin/env python3
"""
Extract Real TEMPO Data for Mexico City
=======================================

Extract real TEMPO L2 data for Mexico City using the correct structure.
"""
...
def main():
    print("🌍 Mexico City TEMPO Data Extractor")
    print("=" * 60)
    print("Extracting real TEMPO L2 data for Mexico City...")
    print()
    
    # Create output directories
    raw_dir = Path("mexico_tempo_raw_complete")
    raw_dir.mkdir(exist_ok=True)
    
    # Mexico City bash scripts
    bash_scripts = [
        "Tempo and weather/Mexico City (Ozone Season Pilot)May 10–15, 2025 (ozone contingency season)/Tempo bash converter  /TEMPO_NO2_L2_V03.sh",
        "Tempo and weather/Mexico City (Ozone Season Pilot)May 10–15, 2025 (ozone contingency season)/Tempo bash converter  /TEMPO_HCHO_L2_V03.sh",
        "Tempo and weather/Mexico City (Ozone Season Pilot)May 10–15, 2025 (ozone contingency season)/Tempo bash converter  /TEMPO_O3TOT_L3_V03.sh"
    ]
    ...
    if all_data:
        print(f"\n🔄 Creating MEXICO_FULL_Pollutant.csv...")
        ...
        if combined_data:
            ...
            # Save to CSV
            output_file = "MEXICO_FULL_Pollutant.csv"
            ...
            if 'latitude' in final_df.columns and 'longitude' in final_df.columns:
                lat_data = final_df['latitude'].dropna()
                lon_data = final_df['longitude'].dropna()
            ...
            for col in final_df.columns:
                if col not in ['filename', 'time'] and final_df[col].dtype in ['float64', 'float32']:
